...
| 4 | **루프 엔지니어링** | 모델이 *스스로 수정하도록* 설계 — 검증 실패를 피드백으로 변환, 자동 재시도 + 반성 루프 |
...
│                                             │
│  context-loader.sh                          │
│  ├─ A3: 문서 압축 주입 (51줄→30줄, -41%)    │
│  ├─ B2: 세션 버퍼 압축 (60줄→1줄, -98%)    │
│  │       + CoT 템플릿 자동 주입             │
│  ├─ 반성 인사이트 (reflection.md) 자동 주입 │
│  └─ 루프 실패 컨텍스트 자동 주입           │
│                                             │
│  ────────────────────────────────────────── │
│                                             │
│  RAG 검색 요청                              │
│  ├─ A2: 쿼리 해시 캐시 (재검색 비용 0)     │
│  └─ B1: MMR 재랭킹 λ=0.5 (중복 페널티)     │
│                                             │
│  gc-agent.sh: 예산 80% 초과 감지           │
│  └─ GC 실행 + 세션 재시작 권고             │
└─────────────────────┬───────────────────────┘
                      │ 커밋 전
                      ▼
          loop-runner.sh --run "constraint-check.sh"
          ┌─────────────────────────────────┐
          │  FAIL → 실패 컨텍스트 기록      │
          │         재시도 (최대 3회)        │◄─┐
          │  C10: 3회 초과 → 태스크 분할   │  │
          ...
          └─ 태스크·루프 히스토리 분석
             → reflection.md 갱신
             → 다음 세션 context-loader.sh 자동 주입
                      │
          ┌───────────┴────────────┐
          │                        │
          ▼                        ▼
    deploy.yml                orchestrate.yml
    (main push 감지)           (PR 오픈·업데이트 감지)
          │                        │
    ghcr.io 이미지                 PR diff 추출
    빌드·푸시                       │
    ...
    (서비스 최신 상태 유지)          ▼
             ...
             ┌────────────────────┼────────────────────┐
             ▼                    ▼                    ▼
      review agent         security agent       test-gen agent
      Claude Sonnet 4.6    Gemini 1.5 Flash     GPT-4o-mini
             └────────────────────┼────────────────────┘
                                  │
                                  ▼
                        ConflictResolver
...
| **L1** | **PEV 루프** — 검증 실패 시 실패 컨텍스트 자동 주입 + 최대 3회 재시도 |
| **L2** | **Reflection Agent** — 세션 종료 후 히스토리 분석, 인사이트를 다음 세션에 자동 주입 |
| **L3** | **C10 Loop Convergence** — 3회 초과 시 태스크 분할 강제, 무한 루프 방지 |
...
├── context-loader.sh     # CoT 템플릿 주입 + 세션 버퍼 로드 + 반성 인사이트 주입
├── doc-compress.sh       # 문서 사전 압축 (A3)
├── constraint-check.sh   # 10개 제약 조건 자동 검증 (C01~C10)
├── gc-agent.sh           # 예산 감시 + 오래된 컨텍스트 GC
├── loop-runner.sh        # PEV 루프 — 검증 자동 재시도 (L1)
└── reflect-agent.sh      # Reflection Agent — 세션 반성 루프 (L2)
```
...
# 커밋 전 제약 검증 (루프 포함)
sh scripts/loop-runner.sh --run "sh scripts/constraint-check.sh"

# 또는 루프 없이 단순 검증
sh scripts/constraint-check.sh

# 세션 종료 전 반성 인사이트 생성
sh scripts/reflect-agent.sh
```
...
**loop-runner.sh: C10 수렴 실패 반복**
```bash
# 루프 상태 확인
sh scripts/loop-runner.sh --status

# 실패 원인 수동 확인
sh scripts/loop-runner.sh --check "검증_명령"

# 상태 초기화 후 태스크 분할 검토
sh scripts/loop-runner.sh --reset
```

**reflect-agent.sh: reflection.md 내용이 오래됨**
```bash
# 수동으로 재생성
sh scripts/reflect-agent.sh

# 분석 내용 미리 확인 (파일 미수정)
sh scripts/reflect-agent.sh --dry-run
```
...
| C10 | 루프 수렴 (3회 초과 시 태스크 분할 강제) | dev+ |
