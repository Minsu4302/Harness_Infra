...
│          │                                                                  │
│  context-loader.sh ──► A3: 문서 압축 주입 (51줄 → 30줄, -41%)              │
│          │          └─► B2: 세션 버퍼 압축 (60줄 → 1줄, -98%) + CoT 주입   │
│          │                                                                  │
│  RAG 검색                                                                   │
│  ├─ A2: 쿼리 해시 캐시 hit → 재검색 비용 0 (문서 변경 시 자동 무효화)       │
│  └─ B1: MMR 재랭킹 (λ=0.5) → 중복 페널티로 검색 다양성 보장                │
│          │                                                                  │
│  커밋 전 constraint-check.sh → C01~C09 자동 검증 (9개 중 8개 완전 자동)    │
│  gc-agent.sh → 예산 80% 초과 감지 시 GC + 세션 재시작 권고                 │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────── Orchestration (GCP e2-micro) ───────────────────────┐
│                                                                             │
│  [개발자 PR 오픈] → GitHub Actions (orchestrate.yml) → PR diff 전송         │
│          │                                                                  │
│  ContextPruner: lock·바이너리 제거, 8,000자 트런케이트                       │
│          │                                                                  │
│  OrchestratorService (Claude Planner)                                       │
│  └─ diff 분석 → 필요한 에이전트·모델만 동적 결정 (불필요 에이전트 스킵)      │
│          │                                                                  │
...
│          │                                                                  │
│  ConflictResolver: review ⊕ security 충돌 시 Claude 재위임 판정             │
│  DeploymentGateService: APPROVED / REJECTED                                 │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
          │
          ▼
...
**CI/CD:** `push to main` → deploy.yml → ghcr.io 이미지 빌드·푸시 → GCP SSH 자동 재배포
